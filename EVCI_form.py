...
# Define the function outptgen
	def outputgen(self):
		...
		for klc in range(1,sm.nlc+1):
			self.m_textfileout.write(f"Nodal Reaction for Loading {klc:3}\nActive Units : {dat[kip][0]:8} {dat[slen][1]:8}\n")
			if sm.strutype == 'Frame3D':
				self.m_textfileout.write("Node     Px          Py          Pz          Mx          My          Mz")
			elif sm.strutype == 'Frame2D   ':
				self.m_textfileout.write("{:10} {:10} {:10} {:10}\n".format('Node'.rjust(10),'Rx'.rjust(10),'Ry'.rjust(10),'Momz'.rjust(10)))
				self.m_textfileout.write("{:10} {:10} {:10} {:10}\n".format('    '.rjust(10),'(kN)'.rjust(10),'(kN)'.rjust(10),'(kN-m)'.rjust(10)))
			for i in range(1,sm.nbn+1):
				l1 = (sm.ndf+1) * (i-1)
				no = sm.ib[l1]
				k1 = sm.ndf * (no-1)+1
				k2 = k1 + sm.ndf
				values = [pylsa.stru3d.reac[j-1, klc-1] for j in range(k1, k2)]
